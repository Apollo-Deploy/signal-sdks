...
/**
 * Client-level credentials inherited from the SDK transport configuration.
 * Applied as defaults to every SSE fetch request; per-request headers take precedence.
 */
export interface SSEClientConfig {
  /** Default headers (auth tokens, API keys, etc.) merged into every SSE request */
  headers?: Record<string, string>;
  /** Default query parameters (auth API keys, etc.) merged into every SSE request */
  query?: Record<string, string>;
  /** Static cookie key/value pairs sent with every SSE request */
  cookies?: Record<string, string>;
  /**
   * Send credentials (cookies, HTTP auth) on cross-origin requests.
   * When true, sets fetch `credentials: 'include'`. Default: same-origin.
   */
 ...
 */
export function parseSSEEventData<T>(
  event: SSEEvent<string>,
  parseData?: <R>(raw: string, type: string) => R,
): SSEEvent<T> {
  if (parseData) {
...
function appendCookie(headers: Record<string, string>, key: string, value: string): void {
 ...
 * ```
 */
export async function* createSSEStream<TData = Record<string, unknown>>(
  url: string,
  init: SSERequestInit,
  ...
  for (const [name, value] of Object.entries(clientConfig?.query ?? {})) {
  ...
  try {
    while (true) {
      ...
      // Apply static cookies from client config
      if (clientConfig?.cookies) {
        for (const [name, value] of Object.entries(clientConfig.cookies)) {
      ...
      try {
        ...
        try {
          ...
          while (true) {
            ...
            for (const rawEvent of rawEvents) {
              ...
              yield parseSSEEventData<TData>(rawEvent, options?.parseData);
            }
          }
        } finally {
          if (heartbeatTimer) clearTimeout(heartbeatTimer);
        ...
        if (heartbeatTimedOut) {
          throw new Error(`SSE heartbeat timed out after ${options?.heartbeatTimeoutMs}ms`);
        }

        // Stream ended cleanly
        if (!options?.reconnectOnComplete) return;
        attempt++;
        if (attempt > rc.maxAttempts) return;
      ...
      if (!(await waitForReconnect(delayMs, externalSignal))) return;
    }
  } finally {
    options?.onClose?.();
  }
...
/**
 * Create an SSE stream that yields only each event's decoded data payload.
 * Used by generated operations with an explicit raw `sseReturnType` override.
 */
export async function* createSSEDataStream<TData>(
  url: string,
  init: SSERequestInit,
  reconnect?: Partial<SSEReconnectConfig>,
  clientConfig?: SSEClientConfig,
  ...
  for await (const event of createSSEStream<TData>(url, init, reconnect, clientConfig, options)) {
    yield event.data;
  }
}
