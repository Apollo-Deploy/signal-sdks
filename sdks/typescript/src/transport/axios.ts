...
/** Fail closed when more than one configured security scheme targets one header. */
export function claimSecurityHeader(claimed: Set<string>, name: string): void {
  const normalized = name.toLowerCase();
  if (claimed.has(normalized)) {
    throw new Error(`Multiple configured security schemes target the ${name} header`);
  }
...
export type ResponseBodyExpectation<T> =
  | { readonly mode: 'unchecked' }
  | ([T] extends [void]
      ? { readonly mode: 'none' }
      : {
          readonly mode: 'required';
          readonly allowEmptyString: '' extends T ? true : false;
          readonly allowNull: null extends T ? true : false;
        });
...
export interface SDKTransport {
  ...
  executeRequest<T>(
    ...
    responseBodyExpectation?: ResponseBodyExpectation<T>,
  ): Promise<T>;
}
...
export function buildSDKUrl(baseUrl: string, requestPath: string): URL {
  const url = new URL(baseUrl);
  url.hash = '';

  const request = new URL(requestPath || '/', 'https://tesseract.invalid');
  const basePath = url.pathname.replace(/\/+$/, '');
  const relativePath = request.pathname.replace(/^\/+/, '');
  url.pathname = `${basePath}/${relativePath}`;

  const requestQueryNames = new Set(request.searchParams.keys());
  for (const name of requestQueryNames) url.searchParams.delete(name);
...
function appendRequestQuery(url: URL, params: unknown): void {
  if (!params) return;
  ...
  const entries =
    params instanceof URLSearchParams
      ...
      : typeof params === 'object'
        ...
        : [];
  const populated = entries.filter(([, value]) => value !== undefined && value !== null);
  const requestQueryNames = new Set(populated.map(([name]) => name));
  for (const name of requestQueryNames) url.searchParams.delete(name);
...
function appendQueryParam(reqConfig: InternalAxiosRequestConfig, key: string, value: string): void {
  const url = new URL(reqConfig.url!);
  url.searchParams.set(key, value);
  reqConfig.url = url.toString();
}

function appendCookie(reqConfig: InternalAxiosRequestConfig, key: string, value: string): void {
  const cookieName = encodeURIComponent(key);
  const serialized = `${cookieName}=${encodeURIComponent(value)}`;
    ...
    typeof existing === 'string'
      ? existing
          ...
          .filter((pair) => {
            ...
            return separator === -1 || pair.slice(0, separator) !== cookieName;
          })
      : [];
  retained.push(serialized);
  reqConfig.headers.Cookie = retained.join('; ');
}
...
export function createTransport(config: TransportConfig): SDKTransport {
  ...
  instance.interceptors.request.use(
    async (reqConfig: InternalAxiosRequestConfig): Promise<InternalAxiosRequestConfig> => {
      ...
      const requestUrl = buildSDKUrl(resolvedConfig.baseUrl, reqConfig.url ?? '');
      appendRequestQuery(requestUrl, reqConfig.params);
      reqConfig.url = requestUrl.toString();
      reqConfig.baseURL = undefined;
      ...
      if (
        ...
        !hasIdempotencyKey(reqConfig.headers)
      ) {
  ...
  const transport: SDKTransport = {
    ...
    executeRequest: <T,>(
      ...
      options?: RequestOptions,
      responseBodyExpectation?: ResponseBodyExpectation<T>,
    ) => executeRequest<T>(transport, requestConfig, options, responseBodyExpectation),
  };
...
export async function executeRequest<T>(
  ...
  requestConfig: AxiosRequestConfig,
  options?: RequestOptions,
  responseBodyExpectation: ResponseBodyExpectation<T> = { mode: 'unchecked' },
): Promise<T> {
  ...
  const mergedConfig: AxiosRequestConfig = {
    ...
    ...(options?.headers ? { headers: { ...requestConfig.headers, ...options.headers } } : {}),
  };
  ...
  for (let attempt = 0; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      // Generated operations declare whether a response body is part of their contract.
      if (response.status === 204 || response.status === 205) {
        if (responseBodyExpectation.mode === 'required') {
          throw new SDKError(
            'Expected a response body for a non-void operation',
            response.status,
      ...
      if (responseBodyExpectation.mode === 'none') {
        return undefined as T;
      }

      // Apply custom response transform if provided
      const data = transport.config.responseTransform
        ? transport.config.responseTransform<T>(response)
        : response.data;

      if (
        responseBodyExpectation.mode === 'required' &&
        (data === undefined ||
          (data === null && !responseBodyExpectation.allowNull) ||
          (data === '' && !responseBodyExpectation.allowEmptyString))
      ) {
        throw new SDKError(
          'Expected a response body for a non-void operation',
          response.status,
...
function mapAxiosError(error: unknown): SDKError {
  ...
  if (!error.response) {
    return new SDKError(error.message || 'Unable to connect to the server', 0, 'network_error');
  }
